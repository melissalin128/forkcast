...
interface RestaurantSeed {
  ...
  distanceMi: number;
  openUntil: string;
  /** Gradient art, shown only if the photo (`assets/img/<id>.jpg`) fails to load. */
  image: string;
  order: OrderLine[];
  orderLabel: string;
...
export const RESTAURANTS: Restaurant[] = SEEDS.map((s) => ({
  ...
  // The slug of a bundled photo; `restaurantPhoto` in lib/photos.ts resolves it to the asset.
  imageUrl: s.id,
  image: s.image,
  order: s.order,
