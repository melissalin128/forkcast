...
import dataset from './generated/restaurants.json';

/**
 * Demo anchor: the instant the bundled dataset was priced. Offers carry it as
 * `fetchedAt`, and the generated history ends here.
 */
export const NOW = new Date(dataset.pricedAt);
export const ZIP = '15213';
/** Zips the bundled restaurants are in (copy only; the fallback does not gate on zip). */
export const COVERED_ZIPS: readonly string[] = dataset.zips;
export const REFRESHED_AT = dataset.pricedAt;
...
// ---------------------------------------------------------------------------
// Restaurants: the bundled export of every priceable restaurant in MongoDB
// (apps/api/src/ingest/exportAppData.ts). Names, photos and menu prices are
// observed; offers (fees, ETAs, totals) are modelled. Full menus live in
// generated/menus.json and are loaded lazily by the Store page.
// ---------------------------------------------------------------------------

const round2 = (n: number) => Math.round(n * 100) / 100;

type ExportedOffer = Omit<Offer, 'restaurantId' | 'subscriptionApplied' | 'promo'>;

/** Fill the Offer fields the export leaves out (same gaps as the live API). */
export const RESTAURANTS: Restaurant[] = (dataset.restaurants as unknown as (Omit<Restaurant, 'offers'> & { offers: ExportedOffer[] })[]).map(
  (r) => ({
    ...r,
    offers: r.offers.map((o) => ({ ...o, restaurantId: r.id, subscriptionApplied: false, promo: undefined })),
  }),
);
