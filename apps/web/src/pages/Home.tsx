...
export function Home() {
  ...
  const {
    deals: liveDeals,
    refreshedAt: dealsRefreshedAt,
    totalActive: dealsTotal,
  } = useDeals();
  ...
  const picks = useMemo(
    () => (hasCombo ? [] : forYou(restaurants, prefs)),
    [hasCombo, restaurants, prefs],
  );
  ...
  const heading =
    category === "All"
      ? `${SORT_WORD[sort]} near you`
      : `${SORT_WORD[sort]} ${CATEGORY_WORD[category]} near you`;
  const count = `${visible.length} place${visible.length === 1 ? "" : "s"}`;
  const combo = queryLabel(category, active, q);
  const passes = prefs.subscriptions.map(
    (s) => PLATFORM_BY_SLUG[s].subscriptionName,
  );
  const passLine =
    passes.length === 0
      ? null
  ...
  return (
    <div className="page">
        ...
        <section className="onboard" aria-label="Set up Forkcast">
          <div>
            ...
            <p className="onboard__sub">
              Add your zip, passes, and taste preferences in Account. Combined
              filters like Italian · under $20 · 30 min live on this feed.
            </p>
          </div>
          <button
            type="button"
      ...
      {!hasCombo && !loading && (
        <DealsStrip promos={promos} restaurants={restaurants} />
      )}
        ...
        <section className="foryou" aria-label="HawtPix picks">
          <div className="deals__head">
            ...
            <span className="deals__sub">
              From your saved places, cuisines and diet — not the crowd
            </span>
          </div>
      ...
      <div className="section-head">
        <div>
          ...
          {passLine && !loading && (
            <span className="section-head__pass">{passLine}</span>
          )}
        </div>
      ...
      {noCoverage ? (
        ...
        <Feed
          ...
          emptyText={emptyText}
          action={
            hasCombo ? { label: "Clear filters", onClick: clearAll } : undefined
          }
        />
      )}
